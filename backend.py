...
def create_assistant():
    assistant = openai.Assistants.create(
        name="SQL and Chat Assistant",
        instructions="You can generate SQL queries for a PostgreSQL database and chat normally.",
        model="gpt-4"
    )
    return assistant.id
...
def get_query_and_table_from_prompt(user_prompt):
    try:
        ...
        run = openai.Runs.create(thread_id=thread_id, assistant_id=assistant_id)
        response_message = run.results[0].messages[0].content.strip()
        query = response_message
        table_name = re.search(r'FROM\s+(\w+)', query, re.IGNORECASE).group(1)
        return query, table_name
    except Exception as e:
        logging.error(f"Error in get_query_and_table_from_prompt: {e}", exc_info=True)
        raise
...
def get_response_from_llm(user_prompt):
    try:
        ...
        logging.error(f"Error in get_response_from_llm: {e}", exc_info=True)
        raise
